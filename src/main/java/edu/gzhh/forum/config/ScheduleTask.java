            ...
            for (Topic topic :topicList){
              String content =   HtmlUtil.cleanHtmlTag(topic.getContent());
                ...
                if (StrUtil.isNotEmpty(result)){
                    ...
                    sensitive.setCid(topic.getTId());
                    sensitive.setType("topic");
                    sensitive.setTopicId(topic.getTId());
                    sensitive.setContent(content);
                    sensitive.setCreateTime(DateUtil.now());
...
//                    sensitiveService.saveSensitiveList(sensitive);
                    sensitiveTopicIds.add(topic.getTId());
                    sensitiveList.add(sensitive);
                }else {
                    noSensitiveTopicIds.add(topic.getTId());
                }
                        }
    ...
    if (ObjectUtil.isNotEmpty(commentLists)){
        ...
        for (Comment comment :commentLists){
            ...
            if (StrUtil.isNotEmpty(result)){
                ...
                Sensitivity sensitive = new Sensitivity();
                sensitive.setCid(comment.getCommentId());
...
//                    sensitiveService.saveSensitiveList(sensitive);
                sensitiveCommentIds.add(comment.getCommentId());
                sensitiveList.add(sensitive);
            }else {
                noSensitiveCommentIds.add(comment.getCommentId());
            }
        }
